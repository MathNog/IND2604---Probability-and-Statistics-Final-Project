...
for year in range(initial_year, final_year):
    print(year)
    dates.append(str(year) + "-01-01 " + str(year) + "-06-01")
    dates.append(str(year) + "-06-01 " + str(year+1) + "-01-01")
...
for i in range(0, len(kw_list), 5):
    ...
    for timeframe in dates:
        ...
        pytrend = TrendReq(backoff_factor=10, retries=5, timeout = (10,25))
    ...
    df_data.to_csv("Probest\TrabalhoFinal\Dados\gtrends_day_finance.csv")
